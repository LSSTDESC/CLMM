...
Now, make sure everything is ok with unittests, notebooks and documentation. These steps (except running the notebooks) are run automatically on each pull request on github (check `details` on `Build and Check / build-gcc-ubuntu (pull_request)` section at the end of the PR and `coverage` under the `coveralls` comment in the middle of the PR), but you can also run them manually on your local machine:
...
9. **Install:** Finally, install (`python setup.py install --user`), run tests (`pytest`) and notbooks (`./run_notebooks`), and compile documentation (`./update_docs`). If everything passes, accept the review!
