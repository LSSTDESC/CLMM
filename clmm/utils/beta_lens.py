...
def compute_beta_s_mean_from_distribution(
    ...
    .. math::
       \left<\beta_s^k\right> = \frac{\int_{z = z_{min}}^{z_{max}}\beta_s^k(z)N(z)}
    ...
    def integrand(z_i):
        return compute_beta_s(z_i, z_cl, z_inf, cosmo) ** k * z_distrib_func(z_i)
...
def compute_beta_s_mean_from_weights(z_src, z_cl, z_inf, cosmo, shape_weights, k=1):
    r"""Mean square value of the geometric lensing efficicency ratio

    .. math::
       \left<\beta_s^k\right> =\frac{\sum_i \beta_s^k(z_i)w_i}
       {\sum_i w_i}
    ...
    return (_shape_weights * beta_s**k).sum() / _shape_weights.sum()
