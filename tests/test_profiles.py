...
def test_instantiate(modeling_data):
    ...
    profile.set_halo_density_profile(halo_profile_model=model.upper(), delta_mdef=200)
    ...
    assert_allclose(profile.rdelta(), cdelta*profile.rscale(), 1e-18)
    assert_allclose(profile.M(profile.rdelta()), mdelta, 1e-18)
    ...
    assert_allclose(profile2.mdelta, 617693839984902.6, reltol)
    assert_allclose(profile2.cdelta, 2.3143737357611425, reltol)

    her = pf.HaloProfileHernquist(mdelta=mdelta, cdelta=cdelta, z_cl=z_cl, cosmo=cosmo)
    assert_allclose(her.M(her.rdelta()), mdelta, 1e-18)
...
def test_einasto(modeling_data):
    ...
    ein2 = ein.to_def('critical', 500)
    assert_allclose(ein2.mdelta, 654444421625520.1, reltol)
    assert_allclose(ein2.cdelta, 2.3593914002446486, reltol)
