...
def test_shear_convergence_unittests(modeling_data):
    ...
    cfg_inf['GAMMA_PARAMS']['z_src_model'] = 'schrabback'
    assert_allclose(theo.compute_reduced_tangential_shear(cosmo=cosmo, **cfg_inf['GAMMA_PARAMS'], beta_s_mean=beta_s_mean, beta_s_square_mean=beta_s_square_mean),
                    (1. + (beta_s_square_mean / (beta_s_mean * beta_s_mean) - 1.) * beta_s_mean * kappa_inf) * (beta_s_mean * gamma_inf / (1. - beta_s_mean * kappa_inf)), 1.0e-10)
    ...
    assert_allclose(mod.eval_reduced_tangential_shear(*profile_pars, 'schrabback', beta_s_mean, beta_s_square_mean), (1. + (beta_s_square_mean / (beta_s_mean * beta_s_mean) - 1.) * beta_s_mean * kappa_inf) * (beta_s_mean * gamma_inf / (1. - beta_s_mean * kappa_inf)), 1.0e-10)
