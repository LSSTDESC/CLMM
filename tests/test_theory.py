...
def test_compute_reduced_shear(modeling_data):
    ...
    assert_allclose(
        theo.compute_reduced_shear_from_convergence(shear[0], convergence[0]),
        truth[0],
        **TOLERANCE,
    )
    assert_allclose(
        theo.compute_reduced_shear_from_convergence(shear, convergence),
        truth,
        **TOLERANCE,
    )
    assert_allclose(
...
def helper_profiles(func):
    ...
    assert_allclose(
        func(r3d, mdelta, cdelta, z_cl, cclcosmo, delta_mdef=200),
        defaulttruth,
        **TOLERANCE,
    )
    assert_allclose(
    ...
    assert_allclose(
        func(r3d, mdelta, cdelta, z_cl, cclcosmo, massdef="mean"),
        defaulttruth,
        **TOLERANCE,
    )
    # Test case fix
...
def test_shear_convergence_unittests(modeling_data, profile_init):
    ...
    def _gt_type0(beta_s_mean, gammat_inf, kappa_inf):
        return beta_s_mean * gammat_inf / (1.0 - beta_s_mean * kappa_inf)

    def _gt_type1(beta_s_mean, beta_s_square_mean, gammat_inf, kappa_inf):
        return beta_s_mean * gammat_inf / (1.0 - beta_s_square_mean / beta_s_mean * kappa_inf)

    def _gt_type2(beta_s_mean, beta_s_square_mean, gammat_inf, kappa_inf):
        return (
    ...
    if profile_init == "nfw" or modeling_data["nick"] in ["nc", "ccl"]:
        ...
        beta_s_mean = compute_beta_s_mean_from_distribution(
            cfg_inf["GAMMA_PARAMS"]["z_cluster"],
            cfg_inf["GAMMA_PARAMS"]["z_src"],
            cosmo,
        )
        ...
        if profile_init == "einasto":
            ...
            if theo.be_nick == "ccl":
                assert_allclose(
                    mod.get_einasto_alpha(cfg["GAMMA_PARAMS"]["z_cluster"]),
                    alpha_ein,
                    reltol,
                )
            if theo.be_nick == "nc":
        ...
        assert_allclose(
            mod.eval_convergence(radius, z_cluster, z_src),
            np.zeros(len(radius)),
            1.0e-10,
        )
        assert_allclose(
            mod.eval_tangential_shear(radius, z_cluster, z_src),
            np.zeros(len(radius)),
            1.0e-10,
        )
        assert_allclose(
        ...
        assert_allclose(
            mod.eval_magnification(radius, z_cluster, z_src),
            np.ones(len(radius)),
            1.0e-10,
        )
        assert_allclose(
...
def test_compute_magnification_bias(modeling_data):
    ...
    assert_allclose(
        theo.compute_magnification_bias_from_magnification(magnification, alpha),
        truth,
        **TOLERANCE,
    )
    assert_allclose(
...
def test_mass_conversion(modeling_data, profile_init):
    ...
    if profile_init == "nfw" or modeling_data["nick"] in ["nc", "ccl"]:
        ...
        assert_allclose(
            profile.eval_mass_in_radius(profile.eval_rdelta(z_cl), z_cl, True),
            mdelta,
            1e-15,
        )
        ...
        assert_raises(
            ValueError,
            profile.convert_mass_concentration,
            z_cl,
