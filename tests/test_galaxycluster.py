...
def test_integrity_of_probfuncs():
    ...
    cluster = clmm.GalaxyCluster(
        ...
        galcat=GCData([ra_source, dec_source, z_sources, id_source],
                      names=('ra', 'dec', 'z', 'id')))
    # true redshift
    cluster.compute_background_probability(add_photoz=False, p_background_name='p_bkg_true')
    expected = np.array([1., 1., 1.])
    assert_allclose(cluster.galcat['p_bkg_true'], expected, **TOLERANCE)

    #photoz + deltasigma
    assert_raises(TypeError, cluster.compute_background_probability, add_photoz=True)
    ...
    cluster.compute_background_probability(add_photoz=True, p_background_name='p_bkg_pz')
...
def test_integrity_of_weightfuncs():
    ...
    cluster.compute_galaxy_weights(cosmo=cosmo, add_shapenoise=False, add_photoz=True,
                                   is_deltasigma=True)
    expected = np.array([9.07709345e-33, 1.28167582e-32, 4.16870389e-32])
    assert_allclose(cluster.galcat['w_ls']*1e20, expected*1e20,**TOLERANCE)

    # test with noise
    cluster.compute_galaxy_weights(cosmo=cosmo, add_shapenoise=True, add_photoz=True,
